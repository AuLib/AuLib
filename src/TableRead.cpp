...
void AuLib::TableReadi::lookup(const double *phs) {
  ...
  double pos, frac, a, b;
  for (uint32_t i = 0; i < m_vframes; i++) {
    pos = mod((m_norm ? phs[i] * m_tframes : phs[i]) + m_phs);
    posi = (uint32_t)pos;
    frac = pos - posi;
    a = m_table[posi];
    b = m_table[posi + 1];
...
void AuLib::TableReadic::lookup(const double *phs) {
  ...
  for (uint32_t i = 0; i < m_vframes; i++) {
    ...
    posi = (uint32_t)pos;
    frac = pos - posi;
    a = posi > 0 ? m_table[posi - 1] : m_table[m_tframes - 1];
    b = m_table[posi];
    c = m_table[posi + 1];
    d = posi < m_tframes ? m_table[posi + 2] : m_table[posi];
    tmp = d + 3.f * b;
    fracsq = frac * frac;
    fracb = frac * fracsq;
    m_vector[i] = fracb * (-a - 3.f * c + tmp) / 6.f +
                  fracsq * ((a + c) / 2.f - b) +
