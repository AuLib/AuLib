...
AuLib::FourierTable::FourierTable(uint32_t harms, double *amps, double phase,
                                  uint64_t tframes)
    : FuncTable(tframes) {
  create(harms, amps, phase);
}

AuLib::FourierTable::FourierTable(uint32_t harms, uint32_t type,
                                  uint64_t tframes)
    : FuncTable(tframes) {
...
void AuLib::FourierTable::create(uint32_t harms, double *amps, double phase) {
  double w, a;
  if (harms) {
    phase *= twopi;
    for (uint32_t i = 0; i < harms; i++) {
